...
int main(int argc, char* argv[]) {
	...
	{											/*Etiqueta para retornar al Menu recursivamente*/
		...
		{				/*Verifica si no hay contactos*/
			...
			switch (x)
			{								/*En este Menï¿½ se validan 2 opciones (Insertar, Salir)*/
			case 1:
		...
		else
		{
			...
			switch (x)
			{								/*Este Menu contiene muchas opciones (Insertar, buscar, listar)*/
			case 1:
				Insertar(Contactos);
				break;
			case 2:
...
void Insertar(struct Agenda Contactos[]) {
	...
	do { 									/*Etiqueta de Menu para insertar contactos*/
		...
		if (ContactosRegistrados < CANTIDAD) {
			...
			archivo << cont;
			archivo.close();
			cout << "\n\tAgregado con exito! (Se agrego el contacto al archivo Agenda.txt)" << endl << endl;
...
void Imprimir(struct Agenda Contactos[], int posicion) {
	...
	case 3:
		if (Salir() == 1)
			return;
	default:
		cout << "\n\tUps, ha ocurrido algo inesperado, presione una tecla para continuar!." << endl;
		Detenerse();
		break;
	}
